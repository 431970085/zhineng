...
public class AdmininfoController {
    @Autowired   //注入对象
    public AdmininfoMapper admininfoMapper;
    @ResponseBody
    ...
    public Object apilist(String keyword, Integer pageIndex, Integer pageSize, Model model) {
        ...
        if (StringUtils.isNotEmpty(keyword)) {
            queryWrapper.like("adminname", keyword); //根据字段搜索查询
        }
        queryWrapper.orderByDesc("adminid");  //根据id降序排列
        IPage<Admininfo> sortPage = new Page<Admininfo>(pageIndex, pageSize);;
        IPage<Admininfo> admininfoIPage = admininfoMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        Map resultMap = new HashMap();
        resultMap.put("list", admininfoIPage.getRecords());
    ...
    public String insert(Admininfo admininfo) {
        admininfoMapper.insert(admininfo);
    ...
    public String list(String keyword, Integer p, Integer pageSize, Model model) {
        ...
        queryWrapper.orderByDesc("adminid");  //根据id降序排列
        IPage<Admininfo> sortPage = new Page<Admininfo>(p, pageSize);
        IPage<Admininfo> admininfoIPage = admininfoMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        model.addAttribute("admininfoList", admininfoIPage.getRecords());   //绑定接受参数
