...
public class CarouselController {
    ...
    public Object apilist(String keyword, Integer pageIndex, Integer pageSize, Model model) {
        ...
        IPage<Carousel> sortPage = new Page<Carousel>(pageIndex, pageSize);;
        IPage<Carousel> carouselIPage = carouselMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        Map resultMap = new HashMap();
        resultMap.put("list", carouselIPage.getRecords());
    ...
    public String list(String keyword, Integer p, Integer pageSize, Model model) {
        ...
        queryWrapper.orderByDesc("id");  //根据id降序排列
        IPage<Carousel> sortPage = new Page<Carousel>(p, pageSize);
        IPage<Carousel> carouselIPage = carouselMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        model.addAttribute("carouselList", carouselIPage.getRecords());   //绑定接受参数
