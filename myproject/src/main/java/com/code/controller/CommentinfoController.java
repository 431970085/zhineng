...
public class CommentinfoController {
    ...
    public GlobalResult apiinsert(Commentinfo commentinfo) {
        try {
            commentinfoMapper.insert(commentinfo);
            return GlobalResult.ok(commentinfo);
        } catch (Exception e) {
    ...
    public GlobalResult apiupdate(Commentinfo commentinfo) {
        try {
    ...
    public Object apilist(String keyword, Integer pageIndex, Integer pageSize, Model model) {
        QueryWrapper<Commentinfo> queryWrapper = new QueryWrapper<Commentinfo>();
        ...
        IPage<Commentinfo> sortPage = new Page<Commentinfo>(pageIndex, pageSize);;
        IPage<Commentinfo> commentinfoIPage = commentinfoMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        Map resultMap = new HashMap();
        resultMap.put("list", commentinfoIPage.getRecords());
    ...
    public String insert(Commentinfo commentinfo) {
        commentinfoMapper.insert(commentinfo);
    ...
    public String list(String keyword, Integer p, Integer pageSize, Model model) {
        ...
        queryWrapper.orderByDesc("id");  //根据id降序排列
        IPage<Commentinfo> sortPage = new Page<Commentinfo>(p, pageSize);
        IPage<Commentinfo> commentinfoIPage = commentinfoMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        model.addAttribute("commentinfoList", commentinfoIPage.getRecords());   //绑定接受参数
