...
public class ScoreinfoController {
    ...
    public Object apilist(String keyword, Integer pageIndex, Integer pageSize, Model model) {
        QueryWrapper<Scoreinfo> queryWrapper = new QueryWrapper<Scoreinfo>();
        ...
        IPage<Scoreinfo> sortPage = new Page<Scoreinfo>(pageIndex, pageSize);;
        IPage<Scoreinfo> scoreinfoIPage = scoreinfoMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        Map resultMap = new HashMap();
        resultMap.put("list", scoreinfoIPage.getRecords());
    ...
    public String list(String keyword, Integer p, Integer pageSize, Model model) {
        ...
        queryWrapper.orderByDesc("id");  //根据id降序排列
        IPage<Scoreinfo> sortPage = new Page<Scoreinfo>(p, pageSize);
        IPage<Scoreinfo> scoreinfoIPage = scoreinfoMapper.selectPage(sortPage, queryWrapper);
        long total = sortPage.getTotal(); //总条数
        long current = sortPage.getCurrent();//当前页
        long pages = sortPage.getPages();//总页数
        model.addAttribute("scoreinfoList", scoreinfoIPage.getRecords());   //绑定接受参数
