...
import com.code.utils.JiebaAnalysisUtil;
...
public class APIController {
    ...
    //分词统计方法
    @ResponseBody
    @RequestMapping("/echarts_word")
    public GlobalResult echartsWordCount(String sql) {
        //String sql = "select name as name from userinfo limit 1000";
        List<Map> maps = commonMapper.selectAction(sql);
        String text = "";
        for (Map datainfo : maps) {
            text = text + datainfo.get("name").toString();
        }
        Map<String, Integer> frequencyMap = JiebaAnalysisUtil.segmentAndCountFrequency(text);
