            ...
            <el-table :data="tableData" size="mini" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                        ...
                        <el-table-column label="用户头像(查看大图)" align="center">
                            <template #default="scope">
                                <el-image class="table-td-thumb" :src="scope.row.headimg" :preview-src-list="[scope.row.headimg]">
                                </el-image>
                            </template>
                        </el-table-column>
                            <el-table-column prop="admincreatetime" label="创建时间"></el-table-column>
                            <el-table-column prop="islock" label="用户状态"></el-table-column>
            ...
            <el-form size="small" label-width="100px">
                            ...
                            <el-form-item label="用户头像:">
                                ...
                                <el-form-item label="用户状态:">
                                    <el-input placeholder="请输入用户状态" v-model="form.islock">
                                    </el-input>
                                </el-form-item>
            </el-form>
            <template #footer>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="45%">
            <el-form size="small" label-width="100px">
                        <el-form-item label="用户编号:">
                            <el-input placeholder="请输入用户编号" :disabled="true" v-model="form.adminid">
                            </el-input>
                        </el-form-item>
                                <el-form-item label="用户账号:">
                                    <el-input placeholder="请输入用户账号" v-model="form.adminname">
    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                ...
                if (keyword != '') {
                    pageNum = 1;
                    sql = "select * from admininfo where adminname like '%" + keyword + "%'  ";
                }
            ...
            const handleDelete = (index, row) => {
                ...
                ElMessageBox.confirm("确定要删除吗？", "提示", {
                    ...
                    .then(() => {
                        //执行删除操作 接口请求
                        var deleteid = row.adminid;
                        var data = {"sql": "delete  from admininfo where  adminid='" + deleteid + "' "}
                        request({
                            url: '/updateAction',
                            method: 'get',
                            params: data
            ...
            const handleAdd = () => {
                ...
                form.admincreatetime = getNowTime();
            };
            const saveInsert = () => {
                ...
                var data = {"sql": `insert into admininfo (adminid,adminname,adminpwd,role,headimg,admincreatetime,islock)  values ('${obj.adminid}','${obj.adminname}','${obj.adminpwd}','${obj.role}','${obj.headimg}','${obj.admincreatetime}','${obj.islock}')`}
                //执行删除操作 接口请求
                request({
                    url: '/updateAction',
                    method: 'get',
            ...
            const saveEdit = () => {
                ...
                var that = this;
                var obj = form;
                var data = {"sql": `  update admininfo set adminname = '${obj.adminname}',adminpwd = '${obj.adminpwd}',role = '${obj.role}',headimg = '${obj.headimg}',admincreatetime = '${obj.admincreatetime}',islock = '${obj.islock}'  where adminid = '${obj.adminid}'  `}
                request({
                    url: '/updateAction',
                    method: 'get',
                    params: data
