    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                ...
                if (keyword != '') {
                    pageNum = 1;
                    sql = "select * from carousel where name like '%" + keyword + "%'  ";
                }
            ...
            const handleDelete = (index, row) => {
                ...
                ElMessageBox.confirm("确定要删除吗？", "提示", {
                    ...
                    .then(() => {
                        //执行删除操作 接口请求
                        var deleteid = row.id;
                        var data = {"sql": "delete  from carousel where  id='" + deleteid + "' "}
                        request({
                            url: '/updateAction',
                            method: 'get',
                            params: data
            ...
            const saveInsert = () => {
                ...
                var data = {"sql": `insert into carousel (id,name,picurl)  values ('${obj.id}','${obj.name}','${obj.picurl}')`}
                //执行删除操作 接口请求
                request({
                    url: '/updateAction',
                    method: 'get',
            ...
            const saveEdit = () => {
                ...
                var that = this;
                var obj = form;
                var data = {"sql": `  update carousel set name = '${obj.name}',picurl = '${obj.picurl}'  where id = '${obj.id}'  `}
                request({
                    url: '/updateAction',
                    method: 'get',
                    params: data
