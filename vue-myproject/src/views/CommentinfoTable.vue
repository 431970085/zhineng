    ...
    <div>
        <div class="container">
            ...
            <el-table :data="tableData" size="mini" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                            ...
                            <el-table-column prop="tid" label="评论数据"></el-table-column>
                            <el-table-column prop="username" label="评论用户"></el-table-column>
                            <el-table-column v-if="false" prop="content" label="评论内容"></el-table-column>
        ...
        <el-dialog title="添加" v-model="addVisible" width="45%">
            <el-form size="small" label-width="100px">
                                ...
                                <el-form-item label="评论内容:" prop="content">
                                    <el-input type="textarea" placeholder="请输入评论内容" rows="5" v-model="form.content"></el-input>
                                </el-form-item>
                            <el-form-item label="用户头像:">
                                <el-input placeholder="请输入用户头像" style="width: 80%;display: inline-block;" v-model="form.picurl">
                                </el-input>
    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                ...
                var sql = " select * from commentinfo  ";
                if (keyword != '') {
                    pageNum = 1;
                    sql = "select * from commentinfo where tid like '%" + keyword + "%'  ";
                }
            ...
            const handleDelete = (index, row) => {
                ...
                ElMessageBox.confirm("确定要删除吗？", "提示", {
                    ...
                    .then(() => {
                        //执行删除操作 接口请求
                        var deleteid = row.id;
                        var data = {"sql": "delete  from commentinfo where  id='" + deleteid + "' "}
                        request({
                            url: '/updateAction',
                            method: 'get',
                            params: data
            ...
            const saveInsert = () => {
                ...
                var data = {"sql": `insert into commentinfo (id,tid,username,content,picurl,addtime,likenum)  values ('${obj.id}','${obj.tid}','${obj.username}','${obj.content}','${obj.picurl}','${obj.addtime}','${obj.likenum}')`}
                //执行删除操作 接口请求
                request({
                    url: '/updateAction',
                    method: 'get',
            ...
            const saveEdit = () => {
                ...
                var that = this;
                var obj = form;
                var data = {"sql": `  update commentinfo set tid = '${obj.tid}',username = '${obj.username}',content = '${obj.content}',picurl = '${obj.picurl}',addtime = '${obj.addtime}',likenum = '${obj.likenum}'  where id = '${obj.id}'  `}
                request({
                    url: '/updateAction',
                    method: 'get',
                    params: data
