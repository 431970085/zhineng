    ...
    <div>
        <div class="container">
            ...
            <el-table :data="tableData" size="mini" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                                            <el-table-column prop="id" label="评分编号"></el-table-column>
                            <el-table-column prop="uid" label="用户编号"></el-table-column>
                            <el-table-column prop="tid" label="数据编号"></el-table-column>
                            <el-table-column prop="score" label="数据评分"></el-table-column>
        ...
        <el-dialog title="添加" v-model="addVisible" width="45%">
            <el-form size="small" label-width="100px">
                                ...
                                <el-form-item label="数据编号:">
                                    <el-input placeholder="请输入数据编号" v-model="form.tid">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="数据评分:">
                                    <el-input placeholder="请输入数据评分" v-model="form.score">
                                    </el-input>
                                </el-form-item>
            </el-form>
            <template #footer>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="45%">
            <el-form size="small" label-width="100px">
                        <el-form-item label="评分编号:">
                            <el-input placeholder="请输入评分编号" :disabled="true" v-model="form.id">
                            </el-input>
                        </el-form-item>
                                <el-form-item label="用户编号:">
                                    <el-input placeholder="请输入用户编号" v-model="form.uid">
    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                ...
                var sql = " select * from scoreinfo  ";
                if (keyword != '') {
                    pageNum = 1;
                    sql = "select * from scoreinfo where uid like '%" + keyword + "%'  ";
                }
            ...
            const handleDelete = (index, row) => {
                ...
                ElMessageBox.confirm("确定要删除吗？", "提示", {
                    ...
                    .then(() => {
                        //执行删除操作 接口请求
                        var deleteid = row.id;
                        var data = {"sql": "delete  from scoreinfo where  id='" + deleteid + "' "}
                        request({
                            url: '/updateAction',
                            method: 'get',
                            params: data
            ...
            const saveInsert = () => {
                ...
                var data = {"sql": `insert into scoreinfo (id,uid,tid,score)  values ('${obj.id}','${obj.uid}','${obj.tid}','${obj.score}')`}
                //执行删除操作 接口请求
                request({
                    url: '/updateAction',
                    method: 'get',
            ...
            const saveEdit = () => {
                ...
                var that = this;
                var obj = form;
                var data = {"sql": `  update scoreinfo set uid = '${obj.uid}',tid = '${obj.tid}',score = '${obj.score}'  where id = '${obj.id}'  `}
                request({
                    url: '/updateAction',
                    method: 'get',
                    params: data
