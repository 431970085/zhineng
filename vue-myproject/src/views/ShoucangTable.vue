    ...
    <div>
        <div class="container">
            ...
            <el-table :data="tableData" size="mini" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                                            <el-table-column prop="id" label="收藏编号"></el-table-column>
                            <el-table-column prop="tid" label="产品编号"></el-table-column>
        ...
        <el-dialog title="添加" v-model="addVisible" width="45%">
            <el-form size="small" label-width="100px">
                                <el-form-item label="产品编号:">
                                    <el-input placeholder="请输入产品编号" v-model="form.tid">
                                    </el-input>
                                </el-form-item>
        ...
        <el-dialog title="编辑" v-model="editVisible" width="45%">
            <el-form size="small" label-width="100px">
                        <el-form-item label="收藏编号:">
                            <el-input placeholder="请输入收藏编号" :disabled="true" v-model="form.id">
                            </el-input>
                        </el-form-item>
                                <el-form-item label="产品编号:">
                                    <el-input placeholder="请输入产品编号" v-model="form.tid">
    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                ...
                if (keyword != '') {
                    pageNum = 1;
                    sql = "select * from shoucang where tid like '%" + keyword + "%'  ";
                }
            ...
            const handleDelete = (index, row) => {
                ...
                ElMessageBox.confirm("确定要删除吗？", "提示", {
                    ...
                    .then(() => {
                        //执行删除操作 接口请求
                        var deleteid = row.id;
                        var data = {"sql": "delete  from shoucang where  id='" + deleteid + "' "}
                        request({
                            url: '/updateAction',
                            method: 'get',
                            params: data
            ...
            const saveInsert = () => {
                ...
                var data = {"sql": `insert into shoucang (id,tid,uid)  values ('${obj.id}','${obj.tid}','${obj.uid}')`}
                //执行删除操作 接口请求
                request({
                    url: '/updateAction',
                    method: 'get',
            ...
            const saveEdit = () => {
                ...
                var that = this;
                var obj = form;
                var data = {"sql": `  update shoucang set tid = '${obj.tid}',uid = '${obj.uid}'  where id = '${obj.id}'  `}
                request({
                    url: '/updateAction',
                    method: 'get',
                    params: data
