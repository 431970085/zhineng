    ...
    export default {
        ...
        setup() {
            ...
            const getData = () => {
                ...
                if (keyword != '') {
                    pageNum = 1;
                    sql = "select * from userinfo where name like '%" + keyword + "%'  ";
                }
            ...
            const handleDelete = (index, row) => {
                ...
                ElMessageBox.confirm("确定要删除吗？", "提示", {
                    ...
                    .then(() => {
                        //执行删除操作 接口请求
                        var deleteid = row.id;
                        var data = {"sql": "delete  from userinfo where  id='" + deleteid + "' "}
                        request({
                            url: '/updateAction',
                            method: 'get',
                            params: data
            ...
            const saveInsert = () => {
                ...
                var data = {"sql": `insert into userinfo (id,name,phone,picurl,email,role,age,sex,password,intro)  values ('${obj.id}','${obj.name}','${obj.phone}','${obj.picurl}','${obj.email}','${obj.role}','${obj.age}','${obj.sex}','${obj.password}','${obj.intro}')`}
                //执行删除操作 接口请求
                request({
                    url: '/updateAction',
                    method: 'get',
            ...
            const saveEdit = () => {
                ...
                var that = this;
                var obj = form;
                var data = {"sql": `  update userinfo set name = '${obj.name}',phone = '${obj.phone}',picurl = '${obj.picurl}',email = '${obj.email}',role = '${obj.role}',age = '${obj.age}',sex = '${obj.sex}',password = '${obj.password}',intro = '${obj.intro}'  where id = '${obj.id}'  `}
                request({
                    url: '/updateAction',
                    method: 'get',
                    params: data
